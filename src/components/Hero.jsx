...
export const Hero = ({ onExploreClick, onOpenQuiz, products }) => {
  const [activeComboId, setActiveComboId] = useState("combo-charcoal-max");
  ...
  const currentCombo =
    combos.find((c) => c.id === activeComboId) ||
  ...
  return (
    <section
      id="hero-section"
      className="relative overflow-hidden pt-6 pb-14 lg:pt-12 lg:pb-20 bg-gradient-to-b from-[#FAF7F2] via-[#F4EFE6] to-[#FAF7F2]"
    >
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div>
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-semibold text-[#1C2C20] leading-[1.1] tracking-tight">
                ...
                <span className="italic font-normal text-[#2E4D36]">
                  Beautifully You.
                </span>
              </h1>
            </div>
            ...
            <p className="text-sm sm:text-base text-[#524B42] leading-relaxed max-w-2xl">
              Switch to chemical-free bathing with Organic Bloom. Handcrafted
              using the authentic Melt &amp; Pour process: pure natural base is
              gently melted down, enriched with real herbs and botanical
              actives, and hand-poured with zero sulfates. Choose our signature
              single bars, or save big with our 3 special value combos:
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-1">
              {[
                ...
                >
                  <span className="block text-[10px] text-[#7B7366] uppercase font-bold tracking-wider">
                    {p.tag}
                  </span>
                  <strong className="font-display text-xs sm:text-sm text-[#1C2C20] block truncate">
                    {p.name}
                  </strong>
                  <span className="text-[10px] font-semibold text-[#C48039]">
                    View Price →
                  </span>
                </button>
              ))}
            ...
            <div className="relative rounded-2xl overflow-hidden bg-[#FAF7F2] border border-[#DDD3C2] shadow-xl p-4 sm:p-5">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    document
                      .getElementById(`product-${currentCombo.id}`)
              ...
              <div
                className={`mt-3.5 grid gap-2 text-center text-xs ${currentCombo.items.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}
              >
                {currentCombo.items.map((it, idx) => (
                  <div
                    key={idx}
                    className="p-2 rounded-lg bg-[#F5EFE6] border border-[#E5DCCF]"
                  >
                    <span className="text-[10px] text-[#786F63] block font-medium">
                      {it.label}
                    </span>
                    <span className="font-bold text-[#1C2C20] text-[11px] block truncate">
                      {it.name}
                    </span>
                  </div>
                ))}
