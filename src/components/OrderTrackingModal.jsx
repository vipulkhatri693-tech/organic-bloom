...
import { findOrder, resubmitPaymentUtr } from "../utils/orderStorage";
import {
  getCurrentUser,
  getOrdersForUser,
  logoutUser,
...
export const OrderTrackingModal = ({
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToComplaints((tickets) => {
      setOrderComplaints(
        tickets.filter((t) => t.orderNumber === activeOrder.orderNumber),
      );
    });
    return unsub;
    ...
    if (user) {
      ...
      if (initialOrderNumber) {
        const match = userOrders.find(
          (o) =>
            o.orderNumber.toLowerCase() === initialOrderNumber.toLowerCase(),
        );
        if (match) {
    ...
    if (currentUser) {
      ...
      const found = userOrders.find((o) => {
        ...
        const trackMatch = Boolean(
          o.trackingId && o.trackingId.toLowerCase() === clean,
        );
  ...
  const handleReorderClick = (items) => {
    if (onReorder) {
      onReorder(items);
    }
    onClose();
  ...
  const handleReportComplaint = (orderNum) => {
    onClose();
    if (onOpenComplaint) {
  ...
  const handleResubmitUtr = (orderNumber, e) => {
    e.preventDefault();
    if (!resubmitUtrValue.trim()) {
      setResubmitError(
        "Please enter your 12-digit UTR / transaction reference number.",
      );
      return;
    }
    ...
    if (cleanUtr.length < 6) {
      setResubmitError(
        "Please enter a valid UTR number (at least 6 characters, standard is 12 digits).",
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (updated) {
        ...
        refreshOrders();
        setResubmitSuccess(
          "New UTR submitted! Host Nikita Khatri has been notified to re-verify.",
        );
        setResubmitUtrValue("");
        setResubmitError(null);
      } else {
        setResubmitError(
          "Could not update UTR. Please try again or message Nikita on WhatsApp.",
        );
      }
    }, 400);
  };
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#696053]">
                Handcrafted Melt &amp; Pour Soaps • Pan-India Courier Updates
        ...
        <div className="flex items-center gap-2 mb-4 bg-[#EDE5D8] p-1 rounded-xl">
          ...
          >
            ...
            <span>
              My Order History {currentUser ? `(${allOrders.length})` : "🔒"}
            </span>
          </button>
        </div>
              ...
              <div className="text-center py-8 bg-white rounded-2xl border border-[#DDD3C2] p-6 space-y-3.5 shadow-xs">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-[#696053] max-w-sm mx-auto leading-relaxed">
                    Your orders and shipping addresses are protected. Please log
                    in with your registered 10-digit mobile number and PIN to
                    access your personal order history.
                  </p>
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-[#F0EAE0] text-[11px] text-[#7A7265] max-w-sm mx-auto">
                  ...
                  <strong className="text-[#263E2E]">98251 44320</strong>{" "}
                  (Pooja) •{" "}
                  <strong className="text-[#263E2E]">98765 43210</strong>{" "}
                  (Aarav) • OTP / PIN: <strong>1234</strong>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between bg-[#E5EFE6] px-3.5 py-2 rounded-xl border border-[#C5DBC7] text-xs">
                  <div className="flex items-center gap-2 text-[#1C2C20]">
                    ...
                    <span>
                      Logged in: <strong>{currentUser.fullName}</strong> (+91{" "}
                      {currentUser.phone})
                    </span>
                  </div>
                  <button
                ...
                {allOrders.length === 0 ? (
                  <div className="text-center py-8 bg-white rounded-xl border border-[#DDD3C2] p-6 space-y-3">
                    ...
                    <p className="text-xs text-[#696053] max-w-sm mx-auto">
                      Explore our handcrafted botanical soaps and place your
                      first order.
                    </p>
                    <button
                      type="button"
                        ...
                        >
                            ...
                            <div className="p-2.5 rounded-lg bg-[#FEF2F2] border border-[#FCA5A5] text-[11px] text-[#991B1B] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 font-medium">
                                ...
                                <span>
                                  ...
                                  {ord.rejectionReason || "UTR not verified"}
                                </span>
                              </div>
                              <button
                                type="button"
                          ...
                          <div className="pt-2 flex flex-wrap items-center gap-2 border-t border-[#F5EFE6]">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleReportComplaint(ord.orderNumber)
                              }
            ...
            <div className="p-3.5 rounded-xl bg-[#EDE5D8] border border-[#DDD3C2] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-[#5D554C]">
                  Explore Charcoal, Rice, or 18-Herbal Bridal Ubtan soap bars
            ...
            <form onSubmit={handleSearch} className="space-y-2">
              ...
              {allOrders.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  <span className="text-[10px] text-[#7A7265] font-semibold">
                    Saved orders:
                  </span>
                  {allOrders.map((ord) => (
                    <button
                  ...
                  {activeOrder.paymentStatus === "rejected" && (
                    <div className="p-4 bg-[#FEF2F2] border-2 border-[#DC2626] rounded-xl text-xs space-y-3 animate-in fade-in">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <p className="text-xs text-[#7F1D1D] leading-relaxed">
                            <strong>Reason from Store Owner:</strong> &ldquo;
                            {activeOrder.rejectionReason ||
                              "UTR number not found in PhonePe statement"}
                            &rdquo;
                          </p>
                      ...
                      <div className="bg-white p-3.5 rounded-xl border border-[#FCA5A5] space-y-2.5 shadow-xs">
                        ...
                        <form
                          onSubmit={(e) =>
                            handleResubmitUtr(activeOrder.orderNumber, e)
                          }
                  ...
                  {activeOrder.paymentStatus ===
                    ...
                    <div className="p-3 bg-[#FFFBEB] border border-[#FCD34D] rounded-lg text-xs space-y-1">
                      ...
                      <p className="text-[11px] text-[#78350F]">
                        ...
                        . Host Nikita Khatri will cross-verify this UTR in her
                        PhonePe app before confirming your batch order.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="p-3 bg-[#FAF6EE] rounded-lg border border-[#E9E1D2] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {activeOrder.trackingId ? (
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] text-[#5A5246]">
                            AWB Tracking #:
                          </span>
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleCopyTracking(activeOrder.trackingId)
                            }
                        ...
                        <div className="text-[11px] text-[#8C5D1E] bg-[#FFF8EE] px-2 py-1 rounded border border-[#EED7B8] font-medium">
                          ⏳ Curing &amp; Handcrafting in progress (2-3 days).
                          Genuine courier tracking number will be assigned by
                          host on dispatch.
                        </div>
                      )}
                ...
                {activeOrder.status === "delivered" && (
                  <div className="p-3 bg-[#FFF8EE] border border-[#FAD7A0] rounded-xl flex items-center justify-between gap-2 shadow-2xs">
                    <div className="flex items-center gap-2 text-xs text-[#7E4C00]">
                      ...
                      <span>
                        Order delivered. If any soap arrived damaged or
                        incorrect, submit a report:
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() =>
                        handleReportComplaint(activeOrder.orderNumber)
                      }
                ...
                <div className="p-3.5 rounded-xl bg-[#FAF6EE] border border-[#DDD3C2] text-xs">
                  ...
                  <div className="space-y-1.5">
                    {activeOrder.items.map((item, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between text-[#38332B]"
                      >
                        <span>
                          {item.quantity}x {item.product.name}
                        </span>
                    ...
                    {orderComplaints
                      ...
                      .map((t) => (
                        ...
                        >
                          ...
                          {t.hostReplyAt && (
                            <span className="text-[10px] text-[#5D5548] mt-1 block">
                              {t.hostReplyAt}
                            </span>
                          )}
                          <p className="text-[10px] text-[#4A433A] mt-2 pt-2 border-t border-[#BFDCC6]/60">
                            Want to reply or attach a photo? Open the{" "}
                            <strong>Care Assistant</strong> chat and tap{" "}
                            <strong>"Track / Reply to Complaint"</strong> with
                            Ticket #{t.ticketId}.
                          </p>
                        </div>
                      ))}
              ...
              <div className="text-center py-8 space-y-3.5 bg-white rounded-2xl border border-[#DDD3C2] p-6 shadow-xs">
                {hasSearched ? (
                  <>
                    ...
                    <p className="text-xs text-[#696053] max-w-sm mx-auto">
                      {currentUser
                        ? "This order number was not found under your registered mobile number (+91 " +
                          currentUser.phone +
                          ")."
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-[#696053] max-w-md mx-auto leading-relaxed">
                        ...
                        <strong>{currentUser.fullName}</strong>. Any handcrafted
                        soap orders placed with this mobile number will appear
                        here automatically.
                      </p>
                    </div>
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-[#696053] max-w-md mx-auto leading-relaxed">
                        Enter your Order Number (e.g. <strong>OB-849201</strong>
                        ) above to check shipment milestones, or log in with
                        your mobile number to view your full order history.
                      </p>
                    </div>
                    <div className="pt-2 flex flex-wrap items-center justify-center gap-2.5">
                      <button
