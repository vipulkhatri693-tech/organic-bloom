...
export const HostDashboardModal = ({ isOpen, onClose, onSwitchToCustomer }) => {
  ...
  // Host Forgot Password state (OTP + reCAPTCHA verified reset)
  const [showHostForgot, setShowHostForgot] = useState(false);
  const [hostForgotStep, setHostForgotStep] = useState("phone");
  const [hostForgotPhone, setHostForgotPhone] = useState("");
  const [hostForgotOtp, setHostForgotOtp] = useState("");
  const [hostForgotConfirmation, setHostForgotConfirmation] = useState(null);
  const [hostForgotNewPassword, setHostForgotNewPassword] = useState("");
  const [hostForgotConfirmPassword, setHostForgotConfirmPassword] =
    useState("");
  ...
  useEffect(() => {
    if (!isOpen) return;
    const unsub = subscribeToComplaints((liveTickets) =>
      setComplaints(liveTickets),
    );
    return unsub;
  }, [isOpen]);
  ...
  const handleHostForgotSendOtp = async (e) => {
    e.preventDefault();
    setHostForgotError(null);

    if (!isFirebaseConfigured) {
      setHostForgotError(
        "OTP verification is not set up yet. Please add your Firebase keys to the .env file.",
      );
      return;
    }
    ...
    try {
      const result = await sendFirebaseOTP(
        clean,
        "ob-recaptcha-host-forgot-container",
      );
      setHostForgotConfirmation(result);
      setHostForgotStep("verify");
      setHostForgotTimer(30);
    } catch (err) {
      resetRecaptcha("ob-recaptcha-host-forgot-container");
      const code = err?.code || "";
      if (code === "auth/invalid-phone-number") {
        setHostForgotError("That mobile number looks invalid.");
  ...
  const handleHostForgotVerifyOtp = async (e) => {
    e.preventDefault();
    setHostForgotError(null);

    if (!hostForgotConfirmation) {
      setHostForgotError("Session expired. Please request the OTP again.");
      setHostForgotStep("phone");
      return;
    }
    if (!hostForgotOtp || hostForgotOtp.trim().length < 6) {
      setHostForgotError(
    ...
    setIsHostForgotSubmitting(true);
    try {
      await hostForgotConfirmation.confirm(hostForgotOtp.trim());
      ...
      if (code === "auth/invalid-verification-code") {
        ...
        setHostForgotError("This OTP has expired. Please request a new one.");
      } else {
        setHostForgotError("Could not verify OTP. Please try again.");
      }
    } finally {
      setIsHostForgotSubmitting(false);
    }
  ...
  const handleHostForgotResetPassword = (e) => {
    e.preventDefault();
    setHostForgotError(null);

    if (
      !hostForgotNewPassword.trim() ||
      hostForgotNewPassword.trim().length < 4
    ) {
    ...
    setIsHostForgotSubmitting(true);
    setTimeout(() => {
      const resetRes = resetHostPassword(
        hostForgotPhone,
        hostForgotNewPassword,
      );
      if (!resetRes.success) {
        setIsHostForgotSubmitting(false);
      ...
      if (loginRes.success && loginRes.host) {
        ...
        setHostForgotConfirmation(null);
      } else {
        setHostForgotError(
  ...
  const handleImageFileChange = (e) => {
    ...
    reader.onload = () => {
      setDraft((d) => (d ? { ...d, image: reader.result } : d));
      setProductFormError(null);
    };
    reader.onerror = () =>
      setProductFormError("Could not read that image file. Please try again.");
  ...
  const handleSaveProduct = (e) => {
    ...
    if (!draft.image.trim()) {
      setProductFormError(
        "Please add a product photo — paste a link or upload a file.",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        isNew
          ? `Added "${finalProduct.name}" to the store.`
          : `Updated "${finalProduct.name}".`,
      );
    } catch (err) {
    ...
    try {
      ...
      showToast(
        announcementActive
          ...
          : "Announcement saved (currently hidden).",
      );
  ...
  const handleConfirmReject = (e) => {
    ...
    setTimeout(() => {
      ...
      if (updated) {
        ...
        refreshData();
        showToast(
          `Payment for #${updated.orderNumber} marked as REJECTED. Customer has been alerted.`,
        );
      }
    }, 300);
  };
  ...
  const handleMarkCraftingDone = (order) => {
    const updated = markSoapCraftingDone(order.orderNumber);
    if (updated) {
      setOrders(getSavedOrders());
      showToast(
        `Order #${order.orderNumber}: Soap Crafting Done ✓ Moved to Courier Pending!`,
      );
    }
  };

  // Handle Action: Revert back to Curing (if marked by mistake)
  const handleRevertToCuring = (order) => {
  ...
  const selectedComplaint =
    complaints.find((t) => t.ticketId === selectedComplaintId) || null;

  const handleSelectComplaint = (ticket) => {
    setSelectedComplaintId(ticket.ticketId);
    setComplaintReplyText("");
  };

  const handleSendComplaintReply = (markResolved) => {
    ...
    setTimeout(() => {
      const updated = addComplaintReply(
  ...
  const handleMarkComplaintResolved = (ticket) => {
  ...
  const filteredComplaints = complaints.filter((t) => {
    ...
    const q = complaintSearchQuery
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
    const hay =
      `${t.orderNumber}${t.ticketId}${t.customerName}${t.customerPhone}`
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
  ...
  const craftingCount = orders.filter(
    (o) => o.status === "crafting" && !isReadyForCourier(o),
  ).length;
        ...
        {toastMessage && (
          <div className="bg-[#1C2C20] text-white px-4 py-2.5 text-xs font-semibold flex items-center justify-between border-b border-[#3B4D3F] shadow-sm animate-fade-in">
            <div className="flex items-center gap-2">
              ...
              {toastMessage.includes("Courier Pending") &&
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#C2B7A7]">
                Manage customer orders, verify PhonePe UTR payments, and assign
                courier tracking
              </p>
            </div>
          </div>
              ...
              {!showHostForgot ? (
                <>
                  <div className="text-center space-y-1.5">
                    ...
                    <p className="text-xs text-[#696053]">
                      Enter your Store Owner ID &amp; Password to access all
                      customer orders and shipment controls.
                    </p>
                  </div>
                ...
                <>
                  <div className="text-center space-y-1.5">
                    ...
                    <p className="text-xs text-[#696053]">
                      Verify your owner mobile number with an SMS code, then set
                      a new password.
                    </p>
                  </div>
                  ...
                  {hostForgotStep === "phone" && (
                    <form
                      onSubmit={handleHostForgotSendOtp}
                      className="space-y-4"
                    >
                      <div>
                        <label className="block text-xs font-bold text-[#38332A] mb-1">
                      ...
                      <div id="ob-recaptcha-host-forgot-container" />
                  ...
                  {hostForgotStep === "verify" && (
                    <form
                      onSubmit={handleHostForgotVerifyOtp}
                      className="space-y-4"
                    >
                      ...
                      <div>
                        ...
                        <div className="flex items-center justify-center gap-1.5 mt-1.5">
                          {hostForgotTimer > 0 ? (
                            ...
                            <button
                              type="button"
                              onClick={(e) => handleHostForgotSendOtp(e)}
                  ...
                  {hostForgotStep === "reset" && (
                    <form
                      onSubmit={handleHostForgotResetPassword}
                      className="space-y-4"
                    >
                      <div className="p-3 bg-[#E5EFE6] border border-[#86EFAC] rounded-xl">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            onChange={(e) =>
                              setHostForgotNewPassword(e.target.value)
                            }
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setShowHostForgotPassword(!showHostForgotPassword)
                            }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setHostForgotConfirmPassword(e.target.value)
                          }
              ...
              <>
                ...
                <div className="bg-white p-4 rounded-xl border border-[#DDD3C2] shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-sm text-[#1C2C20]">
                          {host.name}
                        </span>
                      ...
                      <p className="text-xs text-[#696053]">
                        Connected to store database • {orders.length} total
                        orders recorded
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  ...
                  >
                    ...
                    <span className="text-[10px] text-[#6D28D9]">
                      Tracking AWB assigned
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5">
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      {[
                        ...
                        {
                          key: "crafting",
                          label: `🌿 2-3 Day Curing (${craftingCount})`,
                        },
                        {
                          key: "courier_pending",
                  ...
                  {filteredOrders.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-xl border border-[#DDD3C2] p-6 space-y-2">
                      ...
                      <p className="text-xs text-[#696053]">
                        ...
                        &quot;All&quot; from the filter tabs above.
                      </p>
                      <button
                        type="button"
                        onClick={() => {
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  {isPendingUtr ? (
                                    <span className="bg-[#FEF3C7] text-[#92400E] border border-[#FCD34D] text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 animate-pulse">
                                      <Clock className="w-3 h-3" />
                                      <span>
                                        Action Required: Verify PhonePe UTR
                                      </span>
                                    </span>
                              ...
                              <div className="space-y-0.5">
                                ...
                                {isDispatched && ord.dispatchDate && (
                                  <p className="text-[11px] text-[#554D41]">
                                    Dispatched on: {ord.dispatchDate} •{" "}
                                ...
                                {!isDispatched &&
                                  ord.status === "crafting" &&
                                  isReadyForCourier(ord) &&
                                  ord.curingCompletedAt && (
                                    <p className="text-[10px] text-[#15803D]">
                                      Soap crafting completed on{" "}
                                      {ord.curingCompletedAt}. Awaiting courier
                                      docket assignment.
                                    </p>
                                  )}
                              </div>
                                ...
                                {isPendingUtr && (
                                  <>
                                    ...
                                    >
                                      ...
                                      <span>
                                        ✓ Verify Payment (Matches PhonePe)
                                      </span>
                                    </button>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#7A7265]">
                        Shows as a banner strip at the very top of your site,
                        above the menu.
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[#7A7265]">
                          ...
                          {products.length === 1 ? "" : "s"} — edit prices,
                          photos, descriptions, or add new soaps.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    {products.map((p) => (
                      ...
                      >
                        ...
                        <div className="p-3 space-y-1.5 flex-1 flex flex-col">
                          ...
                          <p className="text-[10px] text-[#7A7265] line-clamp-2">
                            {p.subtitle}
                          </p>
                          <div className="flex items-center gap-1.5 pt-0.5">
                            <span className="text-sm font-bold text-[#263E2E]">
                              ₹{p.price}
                            </span>
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-white border border-[#DDD3C2] shadow-xs">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      <h3 className="text-sm font-bold text-[#1C2C20]">
                        Customer Complaints
                      </h3>
                      <p className="text-xs text-[#786F63] mt-0.5">
                        Tickets raised via the Care Bot — including customers
                        who never had WhatsApp to send their ticket. Search by
                        order ID, ticket ID, name or phone, then reply directly
                        here.
                      </p>
                    </div>
                  ...
                  {filteredComplaints.length === 0 ? (
                    <div className="text-center py-10 text-xs text-[#9A9184]">
                      No complaint tickets{" "}
                      {complaintSearchQuery ? "match your search" : "yet"}.
                    </div>
                  ) : (
                      ...
                      <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
                        {filteredComplaints.map((t) => (
                          ...
                          >
                            ...
                            <div className="text-[10px] text-[#A69C8C] mt-1">
                              {t.createdAt}
                            </div>
                          </button>
                        ))}
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-[11px]">
                              <div className="p-2 rounded-lg bg-white border border-[#E5DCCF]">
                                <span className="block text-[9px] text-[#9A9184] uppercase font-bold">
                                  Order
                                </span>
                              ...
                              <div className="p-2 rounded-lg bg-white border border-[#E5DCCF]">
                                ...
                                <span className="font-mono text-[#1C2C20]">
                            ...
                            <div className="p-3 rounded-lg bg-white border border-[#E5DCCF] text-xs">
                              ...
                              <span className="text-[#2C2926] font-medium">
                                {selectedComplaint.affectedProduct}
                              </span>
                              <span className="block text-[9px] text-[#9A9184] uppercase font-bold mt-2 mb-1">
                                Description
                              </span>
                              <p className="text-[#2C2926] leading-relaxed">
                            ...
                            <div>
                              ...
                              <textarea
                                ...
                                onChange={(e) =>
                                  setComplaintReplyText(e.target.value)
                                }
                                rows={3}
                              ...
                              <div className="flex flex-wrap gap-2 mt-2">
                                <button
                                  type="button"
                                  disabled={
                                    !complaintReplyText.trim() || isSendingReply
                                  }
                                  onClick={() =>
                                    handleSendComplaintReply(false)
                                  }
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#7A7265] mt-1">
                    This exact tracking number will be immediately visible on
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#7A7265] mt-0.5">
                    Leave blank to auto-generate the official courier portal
                    link.
                  </p>
                </div>
            ...
            <div className="w-full max-w-lg bg-white rounded-2xl border border-[#FCA5A5] p-6 shadow-2xl space-y-4">
              ...
              <div className="p-3 bg-[#FAF7F2] rounded-xl border border-[#E9E0D4] space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#1C2C20]">
                    {rejectModalOrder.customer?.fullName} (+91{" "}
                    {rejectModalOrder.customer?.phone})
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[#696053]">
                ...
                <div className="text-[11px] text-[#78350F] bg-[#FFFBEB] p-2 rounded-lg border border-[#FDE68A]">
                  ⚠️ <strong>Customer Notification:</strong> When rejected, the
                  customer&apos;s order tracking portal will immediately show
                  this rejection status and reason, along with an instant UTR
                  resubmission form.
                </div>
              </div>
                    ...
                    {[
                      ...
                      >
                        ...
                        <span className="text-xs font-medium">
                          {reason === "other"
                            ? "Other custom reason (type below)"
                            : reason}
                        </span>
                      </label>
                    ))}
          ...
          <div className="fixed inset-0 z-60 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-fade-in">
            <div className="w-full max-w-2xl max-h-[92vh] bg-white rounded-2xl border border-[#DDD3C2] shadow-2xl flex flex-col overflow-hidden">
              ...
              <form
                ...
                className="flex-1 overflow-y-auto px-5 py-4 space-y-4 text-xs"
              >
                {productFormError && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDraft((d) =>
                          d ? { ...d, weightOz: Number(e.target.value) } : d,
                        )
                      }
                      className="w-full px-3 py-2 bg-white border border-[#DDD3C2] rounded-xl text-[#2C2926] focus:outline-none focus:border-[#263E2E]"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDraft((d) =>
                          d
                            ? { ...d, cureTimeWeeks: Number(e.target.value) }
                            : d,
                        )
                      }
                      className="w-full px-3 py-2 bg-white border border-[#DDD3C2] rounded-xl text-[#2C2926] focus:outline-none focus:border-[#263E2E]"
                  ...
                  <div className="flex items-end pb-2">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        ...
                        onChange={(e) =>
                          setDraft((d) =>
                            d ? { ...d, inStock: e.target.checked } : d,
                          )
                        }
                        className="w-4 h-4 accent-[#263E2E] cursor-pointer"
                      />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setDraft((d) =>
                              d ? { ...d, scentFamily: e.target.value } : d,
                            )
                          }
                          className="w-full px-2.5 py-2 bg-white border border-[#DDD3C2] rounded-xl text-[#2C2926] focus:outline-none focus:border-[#263E2E]"
                        >
                          <option value="Floral & Calming">
                            Floral &amp; Calming
                          </option>
                          <option value="Citrus & Uplifting">
                            Citrus &amp; Uplifting
                          </option>
                          <option value="Earthy & Herbal">
                            Earthy &amp; Herbal
                          </option>
                          <option value="Detox & Clarifying">
                            Detox &amp; Clarifying
                          </option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setDraft((d) =>
                              d
                                ? { ...d, exfoliationLevel: e.target.value }
                                : d,
                            )
                          }
                          className="w-full px-2.5 py-2 bg-white border border-[#DDD3C2] rounded-xl text-[#2C2926] focus:outline-none focus:border-[#263E2E]"
                        >
                          ...
                          <option value="Medium Botanical Scrub">
                            Medium Botanical Scrub
                          </option>
                          <option value="Deep Mineral Scrub">
                            Deep Mineral Scrub
                          </option>
                        </select>
                      </div>
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setDraft((d) =>
                              d ? { ...d, latherProfile: e.target.value } : d,
                            )
                          }
                          className="w-full px-2.5 py-2 bg-white border border-[#DDD3C2] rounded-xl text-[#2C2926] focus:outline-none focus:border-[#263E2E]"
                        >
                          <option value="Ultra Creamy">Ultra Creamy</option>
                          <option value="Velvety Bubbles">
                            Velvety Bubbles
                          </option>
