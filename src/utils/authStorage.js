...
export const resetHostPassword = (phone, newPassword) => {
  const norm = normalizePhone(phone);
  if (norm !== HOST_PHONE) {
    return {
      success: false,
      error: "This mobile number is not registered as the store host.",
    };
  }
...
export const loginHost = (identifier, password) => {
  ...
  const isCorrectPassword = p === getHostPassword();
  ...
  if (isCorrectIdentifier && isCorrectPassword) {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("host_auth_change", { detail: hostData }),
      );
    } catch (e) {
  ...
  if (!isCorrectIdentifier) {
    return {
      success: false,
      error:
        "Invalid Host Mobile Number / ID. Please enter registered owner mobile number (9313268959).",
    };
  }
  ...
  return {
    success: false,
    error:
      'Incorrect Password. Use "Forgot Password?" below to reset it via OTP.',
  };
};
...
export const setCurrentUser = (user) => {
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("user_auth_change", { detail: { user } }),
    );
  } catch (err) {
...
// Generate & Store Pending OTP (4 digits)
export const requestOTP = (phone) => {
  ...
  try {
    ...
    console.error("Failed to store OTP in session:", err);
  }
...
export const verifyOTP = (phone, inputCode, userName) => {
  const norm = normalizePhone(phone);
  ...
  let isValid = false;
  try {
    const raw = sessionStorage.getItem(`ob_otp_${norm}`);
    if (raw) {
  ...
  // Fallback demo codes for convenience during testing
  if (cleanCode === "1234" || cleanCode === "4321") {
    isValid = true;
  }
  ...
  if (!isValid) {
    return {
      success: false,
      error: "Invalid or expired OTP. Please try 1234 or request a new code.",
    };
  }
  ...
  if (!user) {
    ...
    userName &&
    userName.trim() &&
    user.fullName.startsWith("Customer +91")
  ) {
...
export const loginWithPassword = (phoneOrEmail, password) => {
  ...
  const user = users.find((u) => {
    const matchesPhone =
      cleanPhone.length === 10 && normalizePhone(u.phone) === cleanPhone;
  ...
  if (!user) {
    return {
      success: false,
      error:
        "Account not found. Please register with your mobile number and password.",
    };
  }
  ...
  if (user.password && user.password !== password.trim()) {
    return {
      success: false,
      error:
        "Incorrect Password or PIN. Please enter your registered PIN (Default: 1234).",
    };
  }
...
// Caller must have already verified the phone number via Firebase OTP.
export const resetUserPassword = (phone, newPassword) => {
  const norm = normalizePhone(phone);
  if (norm.length !== 10) {
    return {
      success: false,
  ...
  if (clean.length < 4) {
    return {
      success: false,
      error: "Password/PIN must be at least 4 characters.",
    };
  }
...
export const getComplaintTickets = (phone) => {
  try {
    ...
    if (raw) {
      ...
      if (phone) {
        ...
        return sanitized.filter(
          (t) => normalizePhone(t.customerPhone) === norm,
        );
      }
      return sanitized;
    }
...
export const findComplaintTicket = (query) => {
  const q = query
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
...
export const saveComplaintTicket = (ticket) => {
  try {
...
// isn't configured.
export const subscribeToComplaints = (onChange) => {
  const cloudUnsub = subscribeToComplaintsCloud((cloudTickets) => {
    if (cloudTickets.length > 0) {
      try {
        localStorage.setItem(
          "organic_bloom_complaints",
          JSON.stringify(cloudTickets),
        );
      } catch (e) {
...
export const addComplaintMessage = (ticketId, sender, text, options) => {
  try {
    const tickets = getComplaintTickets();
    ...
    const nextStatus = options?.markResolved
      ? "resolved"
      : sender === "host"
        ? "replied"
    ...
    const updated = {
      ...
      status: nextStatus,
      ...(sender === "host"
        ? { hostReply: message.text, hostReplyAt: timestamp }
        : {}),
    };
    tickets[idx] = updated;
    localStorage.setItem("organic_bloom_complaints", JSON.stringify(tickets));
...
export const addComplaintReply = (ticketId, reply, markResolved = false) =>
  addComplaintMessage(ticketId, "host", reply, { markResolved });

export const markComplaintResolved = (ticketId) => {
  try {
    const tickets = getComplaintTickets();
    const idx = tickets.findIndex((t) => t.ticketId === ticketId);
    if (idx === -1) return null;
    const updated = { ...tickets[idx], status: "resolved" };
    tickets[idx] = updated;
    localStorage.setItem("organic_bloom_complaints", JSON.stringify(tickets));
    window.dispatchEvent(new CustomEvent("complaints_updated"));
    pushComplaintToCloud(updated);
