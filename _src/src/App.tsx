...
import { getProducts, subscribeToProducts, subscribeToAnnouncement, type SiteAnnouncement } from './utils/contentStorage';
import { SoapProduct, CartItem } from './types';
...
export default function App() {
  ...
  const [products, setProducts] = useState<SoapProduct[]>(() => getProducts());
  ...
  const [quickViewProduct, setQuickViewProduct] = useState<SoapProduct | null>(null);
  const [isQuizOpen, setIsQuizOpen] = useState(false);
  const [isCheckoutOpen, setIsCheckoutOpen] = useState(false);
  const [isTrackingOpen, setIsTrackingOpen] = useState(false);
  const [isAuthOpen, setIsAuthOpen] = useState(false);
  const [isComplaintOpen, setIsComplaintOpen] = useState(false);
  const [complaintOrderNumber, setComplaintOrderNumber] = useState<string | undefined>(undefined);
  const [trackingOrderNumber, setTrackingOrderNumber] = useState<string | undefined>(undefined);
  const [trackingInitialTab, setTrackingInitialTab] = useState<'history' | 'tracking'>('tracking');
  ...
  const isHostPortalRoute =
    ...
    (window.location.pathname.replace(/\/+$/, '') === '/rev53st-host-portal' ||
     window.location.pathname.includes('rev53st-host-portal') ||
     window.location.hash.includes('rev53st-host-portal') ||
     window.location.search.includes('portal=host'));
  ...
  const handleAddToCart = (product: SoapProduct, quantity: number = 1) => {
    setCart((prevCart) => {
  ...
  const handleOpenCheckout = (promoDiscount: number, promoCode: string, selectedSample: string) => {
    setCheckoutData({
  ...
  const handleOpenTracking = (orderNum?: string, initialTab: 'history' | 'tracking' = 'tracking') => {
