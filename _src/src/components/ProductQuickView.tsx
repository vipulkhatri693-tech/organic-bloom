...
import { X, Star, Check, Plus, Minus, ShoppingBag, Leaf, Droplet, Sprout, ShieldCheck } from 'lucide-react';
      ...
      >
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-8 bg-[#F5EFE6] flex flex-col justify-between border-b md:border-b-0 md:border-r border-[#E6DCCF]">
            ...
            <div className="flex items-center gap-3 mt-4">
              <button
                ...
                className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border-2 transition-all cursor-pointer bg-white ${
                  currentImage === product.image ? 'border-[#263E2E] shadow-xs' : 'border-[#E6DDD0] opacity-75 hover:opacity-100'
                }`}
              >
                ...
                <span className="text-[11px] font-semibold text-[#2C2926]">Poster View</span>
              </button>

              <button
                onClick={() => setSelectedImage(product.secondaryImage!)}
                className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border-2 transition-all cursor-pointer bg-white ${
                  currentImage === product.secondaryImage ? 'border-[#263E2E] shadow-xs' : 'border-[#E6DDD0] opacity-75 hover:opacity-100'
                }`}
              >
                <div className="w-9 h-9 rounded overflow-hidden">
                  <img
                ...
                <span className="text-[11px] font-semibold text-[#2C2926]">Cropped Soap Bar</span>
              </button>
            </div>
          )}
          ...
          <div className="mt-6 pt-4 border-t border-[#E8DFD3] flex items-center justify-between text-xs text-[#5D554C]">
            ...
            {product.cureTimeWeeks > 0 && <span>Aged {product.cureTimeWeeks} Weeks</span>}
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-8 overflow-y-auto max-h-[75vh] md:max-h-[90vh]">
          ...
          <div className="flex items-center gap-2 text-xs text-[#6B645A] mb-1.5">
            ...
            <span>•</span>
            <span className="text-[#263E2E] font-medium">{product.scentFamily}</span>
          </div>

          <h2 className="font-display text-2xl sm:text-3xl font-semibold text-[#1C2C20]">
            {product.name}
          </h2>
          <p className="text-xs text-[#807669] italic mt-0.5">{product.subtitle}</p>

          {/* Pricing */}
          <div className="flex items-baseline gap-2 mt-3 pb-4 border-b border-[#E8DFD3]">
            <span className="font-display text-3xl font-bold text-[#1C2C20]">₹{product.price}</span>
            {product.originalPrice && (
              <span className="text-sm text-[#888176] line-through">₹{product.originalPrice}</span>
            )}
            {product.originalPrice && (
            ...
            <span className="text-xs text-[#605A51] ml-auto">
              {product.category === 'bundle' ? 'Value Combo Pack' : 'Handmade Melt & Pour Bar'}
            </span>
          </div>
          ...
          <div className="mt-5 p-3.5 rounded-xl bg-[#F4EFE6] border border-[#E6DDCE] space-y-2">
            ...
            <div className="text-xs space-y-1">
              <div className="flex gap-2">
                <span className="font-semibold text-[#665F55] w-14 shrink-0">Top:</span>
                <span className="text-[#2C2926]">{product.topNotes.join(', ')}</span>
              </div>
              <div className="flex gap-2">
                <span className="font-semibold text-[#665F55] w-14 shrink-0">Heart:</span>
                <span className="text-[#2C2926]">{product.heartNotes.join(', ')}</span>
              </div>
              <div className="flex gap-2">
                <span className="font-semibold text-[#665F55] w-14 shrink-0">Base:</span>
                <span className="text-[#2C2926]">{product.baseNotes.join(', ')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 pt-4 border-t border-[#E8DFD3]">
            ...
            <span className="text-[10px] text-[#867E73] mt-1 block">
              *Certified Organic &amp; Fair-Trade ingredients. Zero palm oil, SLS, parabens, or synthetic fragrance.
            </span>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-[#E8DFD3] flex items-center gap-3">
            ...
            <div className="flex items-center border border-[#DDD3C2] rounded-full bg-white px-2 py-1">
              ...
              <span className="w-8 text-center text-xs font-bold text-[#2C2926]">{quantity}</span>
              <button
