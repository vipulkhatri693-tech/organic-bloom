...
import { OrderRecord, ComplaintCategory, ComplaintTicket, UserAccount, ComplaintMessage } from '../types';
...
interface ComplaintBotModalProps {
  isOpen: boolean;
  onClose: () => void;
  preselectedOrderNumber?: string;
  onOpenAuth?: () => void;
}

export const ComplaintBotModal: React.FC<ComplaintBotModalProps> = ({
  isOpen,
  onClose,
  ...
  const [allComplaints, setAllComplaints] = useState<ComplaintTicket[]>([]);

  // Bot Flow Steps: 1: select_order, 2: select_category, 3: select_product_details, 4: summary_whatsapp
  const [currentStep, setCurrentStep] = useState<'select_order' | 'select_category' | 'select_product_details' | 'summary_whatsapp'>('select_order');

  // Complaint Form State
  const [selectedOrder, setSelectedOrder] = useState<OrderRecord | null>(null);
  const [manualOrderId, setManualOrderId] = useState('');
  const [category, setCategory] = useState<ComplaintCategory>('damaged');
  ...
  const [mode, setMode] = useState<'new' | 'track'>('track');
  ...
  const [followUpPhotoError, setFollowUpPhotoError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToComplaints((tickets) => {
      ...
      if (trackedTicket) {
        const fresh = tickets.find((t) => t.ticketId === trackedTicket.ticketId);
  ...
  const handleTrackSearch = (e: React.FormEvent) => {
    ...
    if (!found) {
      setTrackError("Couldn't find a complaint with that Order ID or Ticket ID. Double-check and try again.");
  ...
  const handleFollowUpPhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 900 * 1024) {
      setFollowUpPhotoError('That photo is quite large (over ~900KB). Please compress it first or attach a smaller one.');
      return;
    }
    const reader = new FileReader();
    reader.onload = () => setFollowUpPhoto(reader.result as string);
    reader.onerror = () => setFollowUpPhotoError('Could not read that photo. Please try a different file.');
    reader.readAsDataURL(file);
  };

  const handleSendFollowUp = () => {
    if (!trackedTicket || (!followUpText.trim() && !followUpPhoto)) return;
    const textToSend = followUpText.trim() || (followUpPhoto ? 'Photo attached.' : '');
    ...
    if (isOpen) {
      ...
      if (user?.phone) {
        ...
        const userOpen = complaints.filter(
          (t) => normalizePhone(t.customerPhone) === uPhone && t.status !== 'resolved'
        );
      ...
      setMode('new');
      setTrackedTicket(null);
      const deliveredOrder = orders.find((o) => o.status === 'delivered') || orders[0];
      if (deliveredOrder) {
        setSelectedOrder(deliveredOrder);
        setManualOrderId(deliveredOrder.orderNumber);
        setAffectedProduct(deliveredOrder.items[0]?.product.name || 'Entire Parcel');
      }
  ...
  const handleSelectOrderAndProceed = (ord: OrderRecord) => {
  ...
  const handleSelectCategoryAndProceed = (cat: ComplaintCategory) => {
    setCategory(cat);
    setIsBotThinking(true);
    setTimeout(() => {
      setIsBotThinking(false);
  ...
  const handleGenerateComplaintTicket = () => {
    ...
    const custName = currentUser?.fullName || selectedOrder?.customer.fullName || 'Valued Customer';
    const custPhone = currentUser?.phone || selectedOrder?.customer.phone || 'Provided in Chat';
    ...
    const complaintRecord: ComplaintTicket = {
  ...
  const getCategoryTitle = (cat: ComplaintCategory) => {
    switch (cat) {
      case 'damaged':
  ...
  const userPhoneNorm = currentUser?.phone ? normalizePhone(currentUser.phone) : '';
  ...
  const resolvedComplaints = myComplaints.filter((t) => t.status === 'resolved');
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-[#E8DFD3]">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#696053]">
              Customer Support • Fast Replacement &amp; Direct WhatsApp Resolution
            </p>
          </div>
        </div>
            ...
            {openComplaints.length > 0 && (
              <span
                className={`text-[10px] font-extrabold px-1.5 py-0.2 rounded-full ${
                  mode === 'track' ? 'bg-[#3D6347] text-white' : 'bg-[#263E2E] text-white'
                }`}
              >
            ...
            {!trackedTicket ? (
              ...
              <div className="space-y-3">
                {currentUser?.phone ? (
                  ...
                  <div className="space-y-3">
                    <div className="bg-[#FAF4EB] border border-[#E8DFD3] rounded-xl p-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-[#7A7265] mt-0.5">
                          Linked Mobile: +91 {userPhoneNorm} • Real-time Host Replies
                        </p>
                      </div>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#EAF3EC] text-[#15803D] border border-[#BFDCC6]">
                        {openComplaints.length} Open {openComplaints.length === 1 ? 'Chat' : 'Chats'}
                      </span>
                    </div>
                    ...
                    {openComplaints.length > 0 ? (
                      <div className="space-y-2">
                        ...
                        {openComplaints.map((t) => {
                          const hasHostReply = t.status === 'replied' || t.messages.some((m) => m.sender === 'host');
                            ...
                            >
                              ...
                              <div className="text-[11px] text-[#5C5449] bg-white/70 p-2 rounded-lg border border-[#EAE3D7] flex items-center justify-between gap-2 mt-1">
                                <p className="truncate">
                                  {lastMsg ? (
                                    <span>
                                      <strong>{lastMsg.sender === 'host' ? '🌿 Nikita: ' : 'You: '}</strong>
                              ...
                              <div className="flex items-center justify-between text-[9px] text-[#9A9184] mt-2">
                                ...
                                <span>{getCategoryTitle(t.category).split(' ')[1] || 'Support'}</span>
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div className="p-5 rounded-2xl bg-white border border-[#E5DCCF] text-center space-y-3">
                        ...
                        <div>
                          <h3 className="font-bold text-xs text-[#1C2C20]">No Active Support Tickets</h3>
                          <p className="text-[11px] text-[#6E6659] max-w-xs mx-auto mt-1 leading-relaxed">
                            You don't have any open complaints or pending issues for phone +91 {userPhoneNorm}.
                          </p>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="pt-2 border-t border-[#E8DFD3]">
                        ...
                        >
                          <span className="flex items-center gap-1.5">
                            ...
                            <span>Past Resolved Tickets ({resolvedComplaints.length})</span>
                          </span>
                          {showResolvedArchive ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                        </button>
                        ...
                        {showResolvedArchive && (
                          <div className="space-y-2 mt-2">
                            {resolvedComplaints.map((t) => (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <span className="font-bold text-[#1C2C20]">Ticket #{t.ticketId} • Order #{t.orderNumber}</span>
                                  <span className="text-[9px] font-bold text-[#15803D] bg-[#DCFCE7] px-2 py-0.5 rounded-full">
                                    Resolved
                                  </span>
                                </div>
                                <p className="text-[11px] text-[#7A7265] truncate">{t.affectedProduct} — {t.description}</p>
                    ...
                    <div className="pt-1">
                      ...
                      >
                        ...
                        <span>{showManualSearch ? 'Hide manual search' : 'Have a Ticket ID or Order # from another phone?'}</span>
                      </button>

                      {showManualSearch && (
                        <form onSubmit={handleTrackSearch} className="space-y-2 mt-2 p-3 bg-white border border-[#E5DCCF] rounded-xl">
                          <p className="text-[11px] text-[#6E6659]">
                            Enter an Order ID (e.g. OB-84920) or Ticket ID (e.g. OB-CMP-1234):
                          </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-3.5 rounded-xl bg-[#FAF4EB] border border-[#E8DFD3] flex items-start gap-3">
                      ...
                      <div className="space-y-1.5 flex-1">
                        ...
                        <p className="text-[11px] text-[#6E6659] leading-relaxed">
                          If you logged in with your mobile number, all your active complaints and host replies are stored safely in your personal support dashboard.
                        </p>
                        {onOpenAuth && (
                          <button
                            type="button"
                    ...
                    {/* Manual Search Form */}
                    <form onSubmit={handleTrackSearch} className="space-y-2.5 p-3.5 bg-white border border-[#E5DCCF] rounded-xl">
                      <p className="text-[#5A5246] leading-relaxed">
                        Or enter your <strong>Order ID</strong> or <strong>Ticket ID</strong> (e.g. OB-CMP-1234) to see your complaint status and live 1:1 chat:
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
              ...
              <div className="space-y-2.5 flex flex-col h-full">
                ...
                <div className="p-3 rounded-xl bg-white border border-[#E5DCCF] shadow-xs space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      ...
                      {openComplaints.length > 0 && <span>({openComplaints.length} open)</span>}
                    </button>
                  ...
                  <div className="flex items-center justify-between pt-1.5 border-t border-[#F0EAE0]">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-xs text-[#1C2C20]">Nikita Khatri</span>
                          <span className="text-[9px] text-[#15803D] bg-[#EAF3EC] px-1.5 py-0.2 rounded font-medium">
                            Store Host &amp; Care
                          </span>
                        </div>
                        <p className="text-[10px] text-[#7A7265]">
                          Order #{trackedTicket.orderNumber} • {trackedTicket.affectedProduct}
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-3 py-1 overflow-y-auto max-h-[38vh] pr-1">
                  ...
                  <div className="flex items-start gap-2 justify-end">
                    <div className="bg-[#263E2E] text-white p-3 rounded-2xl rounded-tr-xs shadow-xs space-y-1 max-w-[85%]">
                      ...
                      <p className="leading-relaxed text-xs">{trackedTicket.description}</p>
                    </div>
                  </div>
                      ...
                      >
                        <div
                          className={`flex items-center justify-between gap-3 text-[10px] font-semibold ${
                            m.sender === 'host' ? 'text-[#15803D]' : 'text-[#BFD6C5]'
                          }`}
                        >
                          <span className="uppercase">
                            {m.sender === 'host' ? 'Nikita Khatri (Store Host)' : 'You'}
                          </span>
                          <span className="text-[9px] text-[#8C8477] font-normal">{m.timestamp}</span>
                        </div>
                        {m.text && <p className="leading-relaxed text-xs">{m.text}</p>}
                        {m.photo && (
                          <img
                            src={m.photo}
                            alt="Attached proof"
                  ...
                  {trackedTicket.messages.length === 0 && (
                    <div className="p-3 rounded-xl bg-[#FAF7F2] border border-[#E5DCCF] text-center space-y-1">
                      ...
                      <p className="text-[10px] text-[#7A7265] leading-relaxed">
                        Nikita usually replies within 15–30 minutes during business hours. You can type more details or attach photos below.
                      </p>
                    </div>
                  )}
                  ...
                  {trackedTicket.status === 'resolved' && (
                    <div className="p-2.5 rounded-xl bg-[#DCFCE7] border border-[#86EFAC] text-center text-[11px] text-[#15803D] font-medium space-y-0.5">
                      ...
                      <p className="text-[10px] text-[#166534]">
                        Need further assistance? Simply write below to continue the chat!
                      </p>
                    </div>
                  )}
                ...
                <div className="pt-2 border-t border-[#E8DFD3] space-y-2 bg-[#FAF7F2]">
                  ...
                  {followUpPhotoError && (
                    <p className="text-[10px] text-[#C0392B]">{followUpPhotoError}</p>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      disabled={(!followUpText.trim() && !followUpPhoto) || isSendingFollowUp}
                    ...
                    >
                      ...
                      <span>{isSendingFollowUp ? 'Sending...' : 'Send Message'}</span>
                    </button>
                  </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-3.5 pr-1 text-xs">
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="bg-white p-3 rounded-2xl rounded-tl-xs border border-[#E4DCCF] shadow-xs text-[#2C2926] space-y-1 max-w-[85%]">
              ...
              <p className="text-[#5A5246] leading-relaxed">
                Our botanical soaps are carefully handcrafted. If your delivered parcel has any transit damage, wrong item, or quality concern, we will resolve it promptly with a free replacement.
              </p>
            </div>
          </div>
          ...
          {currentStep === 'select_order' && (
            <div className="space-y-3 animate-fade-in pl-9">
              <div className="bg-[#FAF4EB] p-3 rounded-xl border border-[#E8DFD3] space-y-2">
                      ...
                      >
                        <div>
                          ...
                          <span className="text-[10px] text-[#7A7265] block mt-0.5">
                            {(ord.items || []).map((i) => `${i.quantity}x ${i.product?.name || 'Soap'}`).join(', ')}
                          </span>
                        </div>
                ...
                <form onSubmit={handleManualOrderSubmit} className="pt-2 border-t border-[#E8DFD3] flex gap-2">
                  <input
                    type="text"
                    value={manualOrderId}
          ...
          {currentStep === 'select_category' && (
            <div className="space-y-3 animate-fade-in pl-9">
              ...
              <div className="inline-flex items-center gap-2 bg-[#E5EFE6] px-2.5 py-1 rounded-full text-[11px] font-semibold text-[#166534]">
                ...
                <span>Selected: #{selectedOrder?.orderNumber || manualOrderId}</span>
                <button
                  type="button"
              ...
              <div className="bg-[#FAF4EB] p-3 rounded-xl border border-[#E8DFD3] space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() => handleSelectCategoryAndProceed('wrong_item')}
                    className="p-2.5 rounded-lg bg-white border border-[#DDD3C2] hover:border-[#263E2E] hover:bg-[#F9F6F0] text-left transition-colors cursor-pointer space-y-0.5"
                  >
                    <span className="font-bold text-[#1C2C20] block">
                      🔄 Wrong Item Received
                  ...
                  <button
                    type="button"
                    onClick={() => handleSelectCategoryAndProceed('missing_sample')}
                    className="p-2.5 rounded-lg bg-white border border-[#DDD3C2] hover:border-[#263E2E] hover:bg-[#F9F6F0] text-left transition-colors cursor-pointer space-y-0.5"
                  >
                    <span className="font-bold text-[#1C2C20] block">
                  ...
                  <button
                    type="button"
                    onClick={() => handleSelectCategoryAndProceed('skin_reaction')}
                    className="p-2.5 rounded-lg bg-white border border-[#DDD3C2] hover:border-[#263E2E] hover:bg-[#F9F6F0] text-left transition-colors cursor-pointer space-y-0.5"
                  >
                    <span className="font-bold text-[#1C2C20] block">
                  ...
                  <button
                    type="button"
                    onClick={() => handleSelectCategoryAndProceed('delivery_delay')}
                    className="p-2.5 rounded-lg bg-white border border-[#DDD3C2] hover:border-[#263E2E] hover:bg-[#F9F6F0] text-left transition-colors cursor-pointer space-y-0.5"
                  >
                    <span className="font-bold text-[#1C2C20] block">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {selectedOrder.items.map((item, idx) => (
                        <button
                          key={idx}
                          type="button"
                          onClick={() => setAffectedProduct(item.product.name)}
                      ...
                      <button
                        type="button"
                        onClick={() => setAffectedProduct('Entire Package / All Items')}
                        className={`px-2.5 py-1 rounded-md text-[11px] font-medium border cursor-pointer ${
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {[
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setCustomerNote((prev) => (prev ? `${prev} • ${chip}` : chip))
                        }
            ...
            <div className="space-y-3 animate-fade-in pl-9">
              <div className="p-3.5 bg-white rounded-xl border border-[#263E2E] shadow-xs space-y-2.5">
                ...
                <div className="space-y-1 text-[11px] text-[#38332B] bg-[#FAF7F2] p-2.5 rounded-lg border border-[#E8DFD3]">
                  <p>
                    <strong>Order ID:</strong> #{selectedOrder?.orderNumber || manualOrderId}
                  </p>
                  <p>
                    <strong>Issue:</strong> {getCategoryTitle(category)}
                  </p>
                  <p>
                    <strong>Affected Soap:</strong> {affectedProduct || 'Handcrafted Soap'}
                  </p>
                  {customerNote && (
                    <p>
                  ...
                  <p>
                    <strong>Atelier Guarantee:</strong> 100% Doorstep Replacement or Free Refund
                  </p>
                </div>

                <p className="text-[11px] text-[#4A433A] leading-relaxed">
                  Your ticket is saved in our system. Nikita Khatri has received it and can reply directly in your 1-on-1 support chat right here. You can also send it over WhatsApp:
                </p>
                ...
                <button
                  type="button"
                  onClick={() => {
                    const fresh = findComplaintTicket(ticketId) || allComplaints.find((t) => t.ticketId === ticketId);
                    if (fresh) {
                ...
                >
                  ...
                  <span>Send Ticket to Nikita on WhatsApp (+91 93132 68959)</span>
                </a>
