...
import { X, Plus, Minus, Trash2, ShoppingBag, ArrowRight, Sparkles, Tag, Check, Truck, MapPin } from 'lucide-react';
...
interface CartDrawerProps {
  ...
  onOpenCheckout: (promoDiscount: number, promoCode: string, selectedSample: string) => void;
}
...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const [appliedPromo, setAppliedPromo] = useState<{ code: string; percentOff: number; fixedOff: number } | null>(null);
  const [promoError, setPromoError] = useState('');
  const [selectedSample, setSelectedSample] = useState<string>('Rice Soap Mini Sample');
  ...
  if (appliedPromo) {
    if (appliedPromo.percentOff > 0) {
      discountAmount = Math.round((rawSubtotal * appliedPromo.percentOff) / 100);
  ...
  const qualifiesForFreeShipping = rawSubtotal >= FREE_SHIPPING_THRESHOLD || appliedPromo?.code === 'FREESHIP';
  ...
  const handleApplyPromo = (e: React.FormEvent) => {
    ...
    if (code === 'BLOOM10') {
      ...
      setPromoInput('');
    } else if (code === 'ORGANIC50' || code === 'BLOOM50' || code === 'AHMEDABAD') {
          ...
          <div className="flex items-center justify-between text-xs text-[#263E2E] font-medium mb-1.5">
            <div className="flex items-center gap-1.5">
              ...
              {qualifiesForFreeShipping ? (
                <span className="font-bold">You unlocked FREE Pan-India Delivery!</span>
              ) : (
                <span>
                  Add <strong className="font-bold">₹{remainingForFreeShipping}</strong> more for Free Shipping
                </span>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {cart.length === 0 ? (
            <div className="py-16 text-center text-[#736B61] space-y-3">
              ...
              <p className="font-display text-xl text-[#2C2926]">Your bag is currently empty</p>
              <p className="text-xs max-w-xs mx-auto">
                Explore our Rice Soap (₹149), Charcoal Soap (₹100), Bridal Ubtan (₹199), and 4-in-1 Combo (₹449).
              </p>
              <button
                onClick={onClose}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[#7A7267] line-clamp-1">
                    {item.product.category === 'bar' ? 'Handmade soap bar' : 'Value combo pack'}
                  </div>
          ...
          {cart.length > 0 && (
            <div className="pt-2">
              {appliedPromo ? (
                <div className="flex items-center justify-between p-2.5 bg-[#E4ECE4] rounded-lg text-xs text-[#263E2E]">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold">Code applied: {appliedPromo.code}</span>
                    {appliedPromo.percentOff > 0 && <span>(-{appliedPromo.percentOff}%)</span>}
                    {appliedPromo.fixedOff > 0 && <span>(-₹{appliedPromo.fixedOff})</span>}
                  </div>
                  <button
              ...
              {promoError && <p className="text-[11px] text-[#B33927] mt-1">{promoError}</p>}
            </div>
          )}
        </div>
          ...
          <div className="p-5 bg-[#F5EFE6] border-t border-[#E8DFD3] space-y-3">
            <div className="space-y-1.5 text-xs text-[#5D554B]">
              ...
              <div className="flex justify-between">
                ...
                <span>{shippingFee === 0 ? <strong className="text-[#263E2E]">FREE</strong> : `₹${shippingFee}`}</span>
              </div>
            ...
            <button
              ...
              onClick={() => {
                onClose();
                onOpenCheckout(discountAmount, appliedPromo?.code || '', selectedSample);
              }}
            ...
            <p className="text-[10px] text-center text-[#827B70] flex items-center justify-center gap-1.5">
              ...
              <span>Handcrafted Botanical Bars • Pan-India Delivery • 100% Pure Herbs</span>
            </p>
          </div>
        )}
      </div>
