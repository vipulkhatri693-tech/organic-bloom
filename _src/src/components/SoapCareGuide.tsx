...
import { Wind, Droplets, Sun, Check, Plus, ShoppingBag } from 'lucide-react';

interface SoapCareGuideProps {
  products: SoapProduct[];
  onAddToCart: (product: SoapProduct) => void;
}

export const SoapCareGuide: React.FC<SoapCareGuideProps> = ({ products, onAddToCart }) => {
  const [addedAccessoryId, setAddedAccessoryId] = useState<string | null>(null);

  const cedarDish = products.find((p) => p.id === 'accessory-cedar-dish');
  const sisalPouch = products.find((p) => p.id === 'accessory-sisal-pouch');

  const handleAdd = (product: SoapProduct) => {
    onAddToCart(product);
  ...
  return (
    <section id="care-guide" className="py-16 sm:py-24 bg-[#FAF7F2] border-t border-[#E8DFD3]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-sm sm:text-base text-[#5B544B] mt-2.5">
            Natural soaps are rich in moisturizing glycerin that thirsts for air. Follow these simple daily rituals to double each bar’s life.
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-[#E8DFD3]">
            <div>
              ...
              <p className="text-xs text-[#6B6358] mt-0.5">
                Designed specifically to keep Melt &amp; Pour bars dry, firm, and lasting 50% longer.
              </p>
            </div>
          </div>
              ...
              <div className="p-4 rounded-xl bg-white border border-[#DDD3C2] flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6B6358]">{cedarDish.subtitle}</p>
              ...
              <div className="p-4 rounded-xl bg-white border border-[#DDD3C2] flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#6B6358]">{sisalPouch.subtitle}</p>
                    <span className="font-display text-base font-bold text-[#1C2C20] mt-1 block">
                      ₹{sisalPouch.price}
                    </span>
                  </div>
