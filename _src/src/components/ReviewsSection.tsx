...
import { Star, CheckCircle, Plus, MessageCircle, Heart, ShieldCheck, Instagram } from 'lucide-react';
...
export const ReviewsSection: React.FC = () => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('organic_bloom_customer_reviews', JSON.stringify(reviewsList));
    } catch (e) {
  ...
  return (
    <section id="reviews-section" className="py-16 sm:py-20 bg-[#F5EFE6] border-t border-[#E8DFD3]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-6 border-b border-[#E3D9CC] gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#5D554C] mt-1 max-w-2xl">
              We stand by complete authenticity. Every soap bar is handcrafted with pure botanical oils and raw herbs. Share your genuine feedback or message us directly on WhatsApp!
            </p>
          </div>
        ...
        {reviewsList.length === 0 ? (
          <div className="bg-[#FAF7F2] rounded-2xl p-8 sm:p-10 border border-[#DDD3C2] text-center max-w-3xl mx-auto shadow-xs">
            ...
            <p className="text-sm text-[#5D554C] max-w-xl mx-auto leading-relaxed mb-6">
              Unlike mass commercial brands, we have removed all fake testimonials. We craft each Charcoal, Rice, and Bridal Ubtan soap in small batches. When you receive your order, we would love for you to share your genuine experience!
            </p>
                  ...
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-[#787065]">{rev.location || 'India'}</span>
                      </div>
                    </div>
                    <span className="text-[11px] text-[#867E73]">{rev.date}</span>
                  </div>
          ...
          >
            <div className="bg-[#FAF7F2] w-full max-w-lg rounded-2xl p-6 sm:p-8 border border-[#DDD3C2] shadow-2xl relative">
              ...
              <p className="text-xs text-[#6E675D] mb-5">
                We value honest, real customer feedback for our botanical artisanal soaps.
              </p>
              ...
              {formSubmitted ? (
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4 text-xs">
                  <div>
                    <label className="block font-semibold text-[#3D372F] mb-1">Your Full Name</label>
                    <input
                      type="text"
                      required
                      value={newAuthor}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block font-semibold text-[#3D372F] mb-1">City / Location</label>
                      <input
                        type="text"
                        value={newLocation}
                    ...
                    <div>
                      <label className="block font-semibold text-[#3D372F] mb-1">Skin Type</label>
                      <input
                        type="text"
                        value={newSkinType}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white border border-[#DDD3C2] rounded-lg px-3 py-2 text-[#2C2926]"
                    >
                      <option value="Charcoal Soap (₹99)">Charcoal Soap — ₹99 (Deep Detox &amp; Oil Control)</option>
                      <option value="Rice Soap (₹149)">Rice Soap — ₹149 (Tan Removal &amp; Softening)</option>
                      <option value="Bridal Ubtan Soap (₹199)">Bridal Ubtan Soap — ₹199 (18 Ayurvedic Herbs Glow)</option>
                      <option value="Rice + Charcoal Combo (₹179)">Rice + Charcoal Combo — ₹179 (1 Rice + 1 Charcoal)</option>
                      <option value="3-in-1 Daily Radiance Trio (₹399)">3-in-1 Daily Radiance Trio — ₹399 (1 Rice + 1 Charcoal + 1 Ubtan)</option>
                      <option value="4-in-1 Charcoal Max Combo (₹449)">4-in-1 Charcoal Max Combo — ₹449 (2 Charcoal + 1 Rice + 1 Ubtan)</option>
                    </select>
                  </div>

                  <div>
                    <label className="block font-semibold text-[#3D372F] mb-1">Star Rating</label>
                  ...
                  <div>
                    <label className="block font-semibold text-[#3D372F] mb-1">Review Title</label>
                    <input
                      type="text"
                      required
                      value={newTitle}
