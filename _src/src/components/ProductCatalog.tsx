...
import { Sparkles, Eye, Plus, Check, Star, Filter, Heart, ShoppingBag, ShieldCheck } from 'lucide-react';
...
interface ProductCatalogProps {
  ...
  onQuickView: (product: SoapProduct) => void;
...
export const ProductCatalog: React.FC<ProductCatalogProps> = ({
  ...
  const [cardViewMode, setCardViewMode] = useState<Record<string, 'poster' | 'crop'>>({});
  ...
  const handleAddWithFeedback = (product: SoapProduct) => {
    onAddToCart(product);
  ...
  const filteredProducts = useMemo(() => {
    return products
      .filter((p) => {
        ...
        if (activeCategory === 'bundle' && p.category !== 'bundle') return false;

        // Skin type filter
        if (selectedSkinFilter === 'sensitive' || selectedSkinFilter === 'oily') {
          const targetTag = selectedSkinFilter === 'sensitive' ? 'Sensitive' : 'Oily';
          if (!p.skinType.includes(targetTag as any)) return false;
        }
        if (selectedSkinFilter === 'bridal') {
          ...
          if (!(p.id === 'bridal-ubtan-soap' || p.id === 'combo-trio' || p.id === 'combo-charcoal-max')) {
            return false;
          }
        }
        ...
        if (searchQuery.trim()) {
          ...
          const matchBotanicals = p.keyBotanicals.some((b) => b.toLowerCase().includes(q));
          const matchIngredients = p.fullIngredients.some((i) => i.toLowerCase().includes(q));
    ...
    <section id="collection" className="py-16 sm:py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-6 border-b border-[#E8DFD3] gap-4">
        <div>
          ...
          <p className="text-sm text-[#5F584E] mt-2 max-w-2xl">
            Sulfate &amp; paraben free, Melt &amp; Pour artisan soaps crafted in small batches. Pure natural base is melted down and infused with active botanical herbs. Available individually or in high-value combo packs!
          </p>
        </div>

        {/* Sort selector */}
        <div className="flex items-center gap-2 self-start md:self-end">
          <label htmlFor="sort-select" className="text-xs font-medium text-[#655E54] shrink-0">
            Sort by:
          </label>
          <select
            id="sort-select"
      ...
      {filteredProducts.length === 0 && (
        <div className="text-center py-16 bg-[#FAF7F2] rounded-2xl border border-[#E8DFD3]">
          <p className="font-display text-2xl text-[#2C2926]">No products found</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-7">
        {filteredProducts.map((product) => {
          ...
          const displayedImage = isCropped && product.secondaryImage ? product.secondaryImage : product.image;
              ...
              <div className="relative aspect-[4/3] bg-[#EFE9DF] overflow-hidden">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      setCardViewMode((prev) => ({
                        ...prev,
                        [product.id]: prev[product.id] === 'crop' ? 'poster' : 'crop',
                      }));
                    }}
                ...
                <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center">
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-black/60 backdrop-blur-md text-white">
                    {product.category === 'bundle' ? 'Value Pack' : 'Handmade Bar'}
                  </span>
                ...
                <div className="mt-5 pt-3.5 border-t border-[#E8DFD3] flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-[#7A7368]">
                      {product.category === 'bar' ? '100% Pure Natural Herbs' : 'Combo Discount Pack'}
                    </span>
                  </div>
      ...
      <div className="mt-12 bg-gradient-to-r from-[#263E2E] to-[#1C2C20] text-white rounded-2xl p-6 sm:p-8 shadow-lg border border-[#3E5C46] flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-[#C4D9C8] max-w-xl">
            Get <strong>2 Charcoal Soaps + 1 Rice Soap + 1 Bridal Ubtan Soap</strong> in one curated box. Deep pore detox for the week, daily rice brightening, and traditional wedding-level haldi chandan glow on weekends.
          </p>
          <div className="flex flex-wrap gap-4 text-xs text-[#E3B873] pt-1">
            ...
            <span className="font-bold text-white bg-[#34533D] px-2 py-0.5 rounded">You Pay Only ₹449</span>
          </div>
        </div>
