...
import { X, CheckCircle2, ShieldCheck, MapPin, Lock, ArrowRight, MessageCircle, Copy, Check, QrCode, Phone, ExternalLink, Sparkles, Truck, Mail, FileText, ShoppingBag } from 'lucide-react';
import { BRAND_INFO } from '../data/soaps';
import { PhonePeQRCard } from './PhonePeQRCard';
import { saveOrderRecord, generateTrackingMilestones } from '../utils/orderStorage';
import { getCurrentUser, registerUser, normalizePhone } from '../utils/authStorage';
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    state: '',
    pincode: '',
    paymentMethod: 'phonepe' as 'cod' | 'gpay' | 'phonepe' | 'upi',
    orderNotes: '',
  });
  ...
  const rawSubtotal = cart.reduce((s, i) => s + i.product.price * i.quantity, 0);
  const subtotal = Math.max(0, rawSubtotal - promoDiscount);
  const shipping = rawSubtotal >= 499 || promoCode === 'FREESHIP' ? 0 : 40;
  const total = subtotal + shipping;

  const generateWhatsAppMessage = (ordNum: string, txId?: string, isPaid?: boolean, chosenMethod?: string) => {
    const itemsList = cart.map((item) => `• ${item.quantity}x ${item.product.name} (₹${item.product.price * item.quantity})`).join('\n');
    const finalMethod = chosenMethod || formData.paymentMethod || 'phonepe';
    const payModeName = 
      finalMethod === 'cod' ? 'Cash on Delivery (COD)' :
      finalMethod === 'gpay' ? 'Google Pay (GPay)' :
      finalMethod === 'phonepe' ? 'PhonePe (UPI QR)' : 'UPI Transfer';

    const paymentStatusText = finalMethod === 'cod'
      ? 'Cash on Delivery (COD at Doorstep) 📦'
      : `PhonePe / UPI (Submitted UTR: ${txId || 'Pending'}) ⏳ Host Verification Required`;

    const text = 
`*🌿 NEW ORDER — ORGANIC BLOOM*
*Order No:* ${ordNum}
...
Please verify in PhonePe App / HDFC Account 6686 to confirm this order. Thank you!`;
  ...
  const executeOrder = (options?: { isPaid?: boolean; txId?: string; chosenMethod?: 'cod' | 'phonepe' | 'gpay' | 'upi' }) => {
    ...
    if (finalMethod !== 'cod' && (!finalTxId || finalTxId.length < 6)) {
      setValidationError('Please enter your 12-digit UTR No. or Transaction ID from PhonePe to submit your order.');
    ...
    setTimeout(() => {
      ...
      const newOrder: OrderRecord = {
        ...
        paymentStatus: finalMethod === 'cod' ? 'pending_cod' : 'pending_host_verification',
        ...
        trackingId: '',
        trackingUrl: '',
        estimatedDelivery: 'Curing in progress (2-3 days). Tracking provided upon dispatch.',
  ...
  const handleQrPaymentSuccess = (info: { transactionId?: string; upiId: string }) => {
    executeOrder({
      isPaid: true,
        ...
        {isConfirmed ? (
          ...
          <div className="text-center py-2 space-y-4 animate-scale-up">
            ...
            <div className="space-y-1">
              ...
              <h2 className="font-display text-2xl sm:text-3xl font-semibold text-[#1C2C20]">
                Thank You, {formData.fullName.split(' ')[0] || 'Valued Customer'}!
              </h2>
              <p className="text-xs sm:text-sm text-[#5D554C] max-w-md mx-auto">
                Order <strong className="font-mono text-[#263E2E] bg-white px-2 py-0.5 rounded border border-[#DDD3C2]">{orderNumber}</strong> has been received by Nikita Khatri.
              </p>
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-[#FAF6EE] border border-[#E9DFCF] text-left max-w-lg mx-auto space-y-2 text-xs">
              ...
              {transactionId && (
                <div className="bg-white p-2 rounded-lg border border-[#E2D8C7] flex items-center justify-between text-xs">
                  <div>
                    <span className="text-[10px] text-[#7A7265] block">Submitted UTR / Txn ID</span>
                    <span className="font-mono font-bold text-[#5F259F] text-xs">{transactionId}</span>
                  </div>
              ...
              <p className="text-[11px] text-[#6B6459] leading-relaxed">
                Host Nikita Khatri will verify your transaction in the PhonePe Business app. Once verified, your order status will update to <strong>Payment Confirmed</strong>.
              </p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-white border border-[#DDD3C2] shadow-xs text-left max-w-lg mx-auto space-y-3">
              ...
              <div className="bg-[#FAF7F2] p-3 rounded-lg border border-[#EBE3D7] text-xs space-y-1.5">
                ...
                <p className="text-[11px] text-[#6B6459] leading-relaxed">
                  We do not generate fake tracking links. Handcrafted herbal soaps require 2-3 days for curing and hermetic moisture-sealing. Once your parcel is handed over to the courier (Delhivery, Blue Dart, or India Post), the host will input the genuine courier tracking number in the system, and it will appear in your Tracking Portal immediately.
                </p>
              </div>
          ...
          <div>
            ...
            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#4A433A] mb-1">
                  Complete Delivery Address (House No., Flat/Society, Street, Area) *
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#4A433A] mb-1">City *</label>
                  <input
                    type="text"
                    required
                    value={formData.city}
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#4A433A] mb-1">State *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#4A433A] mb-1">PIN Code *</label>
                  <input
                    type="text"
                    required
                    placeholder="380015"
              ...
              <div className="pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  >
                    ...
                    <p className={`text-[11px] leading-tight ${formData.paymentMethod === 'phonepe' ? 'text-[#D0DFC9]' : 'text-[#6B6459]'}`}>
                  ...
                  >
                    ...
                    <p className={`text-[11px] leading-tight ${formData.paymentMethod === 'gpay' ? 'text-[#D0DFC9]' : 'text-[#6B6459]'}`}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                        formData.paymentMethod === 'cod' ? 'bg-[#E3B873] text-[#1C2C20]' : 'bg-[#EAE2D5] text-[#4A433A]'
                      }`}>
                        COD
                      </span>
                    </div>
                    <p className={`text-[11px] leading-tight ${formData.paymentMethod === 'cod' ? 'text-[#D0DFC9]' : 'text-[#6B6459]'}`}>
                  ...
                  <div className="mt-3 animate-fade-in">
                    <div className="mb-2 px-3 py-2 bg-[#FAF4EA] rounded-xl border border-[#E2D8C7] text-xs text-[#4A4339] flex items-center justify-between">
                      <span className="font-medium">
                        💡 Scan the QR code or tap Pay via PhonePe. Copy the 12-digit UTR No. and submit below so the host can verify and approve your order.
                      </span>
                    </div>
                    <PhonePeQRCard
                      amount={total}
              ...
              <div className="p-3.5 rounded-xl bg-[#F5EFE6] border border-[#E4DCCF] text-xs space-y-1">
                ...
                <div className="flex justify-between text-[#5E574E]">
                  ...
                  <span>{shipping === 0 ? <strong className="text-[#263E2E]">FREE</strong> : `₹${shipping}`}</span>
                </div>
              ...
              <div className="pt-2">
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      <span>
                        Complete Order — ₹{total} ({formData.paymentMethod === 'cod' ? 'Cash on Delivery' : 'PhonePe / UPI'})
                      </span>
