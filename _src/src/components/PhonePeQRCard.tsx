...
interface PhonePeQRCardProps {
  ...
  orderNumber?: string;
  className?: string;
  onPaymentReport?: () => void;
  onPaymentSuccess?: (info: { transactionId?: string; upiId: string }) => void;
}
...
export const PhonePeQRCard: React.FC<PhonePeQRCardProps> = ({
  ...
  const [selectedUpiId, setSelectedUpiId] = useState<string>('khatrinikita03@ybl');
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [qrDataUrl, setQrDataUrl] = useState<string>('/images/phonepe-qr.svg');
  ...
  const note = encodeURIComponent(`Organic Bloom ${orderNumber ? `#${orderNumber}` : 'Soap Order'}`);
  ...
  const handleConfirmPayment = () => {
    ...
    if (!cleanUtr || cleanUtr.length < 6) {
      setUtrError('Please enter your 12-digit UTR or Transaction ID from PhonePe.');
      return;
    }
    setUtrError(null);
    setIsVerifying(true);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between pb-3 border-b border-[#222228]">
        <div>
          <h3 className="text-base font-bold text-white leading-tight">Receive Money</h3>
      ...
      <div className="my-3 py-2 px-3 bg-[#1C1C22] rounded-xl border border-[#2B2B32] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-xs font-bold text-white block">HDFC Bank - 6686</span>
            <span className="text-[10px] text-[#A0A0AA] block">Payee: Nikita Khatri</span>
          </div>
        </div>
        {amount && (
          <div className="text-right">
            <span className="text-[10px] text-[#9E9EA5] block uppercase tracking-wider">Amount Due</span>
      ...
      <div className="relative bg-white p-4 rounded-xl shadow-inner mx-auto w-56 sm:w-60 aspect-square flex items-center justify-center my-3">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-11 h-11 rounded-full bg-[#5F259F] border-[3px] border-white flex items-center justify-center shadow-md">
            <span className="text-white font-black text-lg leading-none select-none">पे</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-3.5 bg-[#1C1C24] rounded-xl border border-[#353542] mb-3 space-y-2.5">
        ...
        <div>
          ...
          <p className="text-[10px] text-[#9090A0] mt-1 leading-normal">
            ℹ️ The host will cross-verify this UTR in PhonePe before confirming and dispatching your hand-cured soaps.
          </p>
        </div>
      ...
      <div className="pt-3 border-t border-[#222228] space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-[#A5A5AF] tracking-wider uppercase">UPI IDs</span>
          <span className="text-[10px] text-[#9D65E0] font-semibold">Click to Copy / Select</span>
        </div>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-left">
                    <span className="font-mono text-xs font-bold block text-white">{item.id}</span>
                    <span className="text-[10px] text-[#858590] block">{item.label}</span>
                  </div>
                </div>
