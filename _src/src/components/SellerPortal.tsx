...
import { OrderRecord, OrderStatus, HostAccount } from '../types';
import {
  getSavedOrders,
  verifyPaymentByHost,
  rejectPaymentByHost,
...
export const SellerPortal: React.FC<SellerPortalProps> = ({ onSwitchToCustomer }) => {
  const [host, setHost] = useState<HostAccount | null>(null);
  const [orders, setOrders] = useState<OrderRecord[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [activeFilter, setActiveFilter] = useState<
    'all' | 'pending_verification' | 'rejected' | 'crafting' | 'shipped' | 'delivered'
  >('all');
  ...
  const handleSellerLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (res.success && res.host) {
        ...
        setLoginError(res.error || 'Invalid seller credentials. Check mobile number & password.');
      }
    }, 400);
  };
  ...
  const handleConfirmReject = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (updated) {
        ...
        refreshData();
        showToast(`Payment for #${updated.orderNumber} marked as REJECTED. Customer alerted.`);
      }
    }, 300);
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `OrganicBloom_Orders_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div id="seller-hub-root" className="min-h-screen w-full bg-[#FAF7F2] flex flex-col font-sans text-[#2C2926]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#C2B7A7] hidden sm:block">
              Artisan Operations • PhonePe UTR Settlement &amp; Dispatch Logistics
            </p>
          </div>
        </div>
              ...
              <div className="bg-[#1C2C20] text-white p-8 sm:p-10 flex flex-col justify-between space-y-6">
                <div className="space-y-3">
                  ...
                  <p className="text-xs sm:text-sm text-[#C2B7A7] leading-relaxed">
                    The central command system for managing handcrafted soap orders, reconciling PhonePe UPI payments, tracking herbal batch curing schedules, and booking express courier shipments.
                  </p>
                </div>
                ...
                <div className="space-y-3 text-xs text-[#E5EFE6]">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span>1-Click PhonePe UTR Match &amp; Settlement Approval</span>
                  </div>
                  <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span>Courier Tracking Assignment (Delhivery &amp; BlueDart)</span>
                  </div>
                  <div className="flex items-center gap-2.5">
              ...
              <div className="p-8 sm:p-10 flex flex-col justify-center space-y-6 bg-white">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-[#696053]">
                    Enter your Store Owner mobile number &amp; password to access all orders.
                  </p>
                </div>
                ...
                <form onSubmit={handleSellerLoginSubmit} className="space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-xs font-bold text-[#38332A]">Seller Password *</label>
                      <button
                        type="button"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#DDD3C2] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-base text-[#1C2C20]">{host.name}</span>
                  ...
                  <p className="text-xs text-[#696053]">
                    Mobile: +91 {host.phone} • {orders.length} total customer orders recorded
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
              ...
              >
                ...
                <span className="text-[11px] text-[#92400E]">Verify in PhonePe</span>
              </div>
              ...
              >
                ...
                <span className="text-[11px] text-[#55695A]">Artisan drying &amp; curing</span>
              </div>
              ...
              >
                ...
                <span className="text-[11px] text-[#55695A]">Completed orders</span>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-white border border-[#DDD3C2] shadow-2xs col-span-2 sm:col-span-1">
                ...
                <span className="text-[11px] text-[#7A7265]">Paid &amp; verified total</span>
              </div>
            </div>
            ...
            {filteredOrders.length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-[#DDD3C2] text-center space-y-3">
                ...
                <h3 className="font-display text-lg font-bold text-[#1C2C20]">No Orders Found</h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                        ...
                        <div className="space-y-1.5 p-3 rounded-xl bg-[#FAF7F2] border border-[#EBE4D8]">
                          <span className="text-[10px] uppercase font-bold text-[#7A7265] block">
                            🧼 Ordered Soaps &amp; Combos ({ord.items?.length || 0})
                          </span>
                          <div className="space-y-1 max-h-32 overflow-y-auto pr-1">
                            {ord.items?.map((it, idx) => (
                              <div key={idx} className="flex items-center justify-between text-xs">
                                <span className="text-[#1C2C20]">
                                  {it.product.name} × <strong className="font-mono">{it.quantity}</strong>
                                </span>
                          ...
                          <div className="pt-1.5 border-t border-[#DDD3C2] flex items-center justify-between font-bold text-xs text-[#1C2C20]">
                            ...
                            <span className="text-sm font-mono text-[#263E2E]">₹{ord.total}</span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 p-3 rounded-xl bg-[#FAF7F2] border border-[#EBE4D8]">
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-[#554D41]">Payment Status:</span>
                            <span
                          ...
                          {ord.paymentStatus === 'rejected' && (
                            <div className="mt-1 p-1.5 rounded bg-white border border-[#FCA5A5] text-[10px] text-[#991B1B] space-y-0.5">
                              ...
                              <p>
                                <strong>Reason:</strong> {ord.rejectionReason || 'UTR not found in PhonePe'}
                              </p>
                        ...
                        <div className="space-y-0.5">
                          ...
                          {isDispatched && ord.dispatchDate && (
                            <p className="text-[11px] text-[#554D41]">
                              Dispatched: {ord.dispatchDate} • Est: {ord.estimatedDelivery}
                            </p>
                          )}
                        </div>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-[#F0EBE1]">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            ...
                            <span>
                              {isDispatched ? 'Update Tracking #' : '📦 Assign Courier & Dispatch'}
                            </span>
                          </button>
                        </div>
        ...
        <div className="fixed inset-0 z-60 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl border border-[#DDD3C2] space-y-4">
            ...
            <form onSubmit={handleSubmitTracking} className="space-y-4 text-xs">
              <div>
                <label className="block font-bold text-[#38332A] mb-1">Courier Partner *</label>
                <select
                  value={courierName}
              ...
              <div>
                ...
                <p className="text-[10px] text-[#7A7265] mt-1">
                  This tracking number will immediately appear on the customer&apos;s order tracking screen.
                </p>
              </div>
          ...
          <div className="w-full max-w-lg bg-white rounded-3xl border border-[#FCA5A5] p-6 shadow-2xl space-y-4">
            ...
            <div className="p-3 rounded-xl bg-[#FFF5F5] border border-[#FED7D7] text-xs text-[#9B2C2C] space-y-1">
              ...
              <p className="text-[11px] text-[#C53030]">
                Marking this payment as rejected will immediately alert the customer on their tracking screen and prompt them to re-enter a valid 12-digit UTR or send payment screenshot.
              </p>
            </div>
                ...
                <a
                  href={`https://wa.me/91${normalizePhone(rejectModalOrder.customer?.phone)}?text=${encodeURIComponent(
                    `Hi ${rejectModalOrder.customer?.fullName || 'Customer'}! Regarding your Organic Bloom Order #${rejectModalOrder.orderNumber}: We were unable to verify your PhonePe payment reference (${rejectModalOrder.utrNumber || 'None'}). Reason: ${
                      rejectReasonPreset === 'other' ? customRejectReason : rejectReasonPreset
