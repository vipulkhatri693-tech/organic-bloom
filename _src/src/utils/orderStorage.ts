import { OrderRecord, OrderStatus, TrackingMilestone, CartItem } from '../types';
...
export const generateTrackingMilestones = (
  ...
  const isCompletedOrPassed = (targetStatus: OrderStatus) => {
  ...
  return [
    {
      ...
      description: 'Order received. Payment details & UTR submitted for host verification.',
    ...
    },
    {
      ...
      description: 'Host verified transaction via PhonePe / Bank. Order scheduled for batch creation.',
    ...
    {
      status: 'shipped',
      label: hasDispatched ? `Dispatched via ${courierName || 'Courier'}` : 'Courier Dispatch & AWB Generation',
      ...
      timestamp: hasDispatched ? getFormattedDate(-10) : 'Pending Curing Completion',
      location: hasDispatched ? `${courierName || 'Courier'} Air Cargo Hub` : 'Packaging Facility',
...
export const getSampleDemoOrders = (): OrderRecord[] => {
  return [
    {
      ...
      milestones: generateTrackingMilestones('placed', 'Noida', getFormattedDate(-2)),
    },
    {
      ...
      milestones: generateTrackingMilestones('crafting', 'Mumbai', getFormattedDate(-18)),
    },
    {
      ...
      milestones: generateTrackingMilestones('delivered', 'Ahmedabad', getFormattedDate(-28)),
    },
  ];
};
...
const sanitizeOrder = (ord: any): OrderRecord => ({
  orderNumber: ord.orderNumber || `OB-${Math.floor(100000 + Math.random() * 900000)}`,
  ...
  total: typeof ord.total === 'number' ? ord.total : (ord.subtotal || 0) + (ord.shipping || 0),
  ...
  curingCompleted:
    ord.curingCompleted === true ? true : ord.curingCompleted === false ? false : undefined,
  curingCompletedAt: ord.curingCompletedAt || undefined,
  curingCompletedAtMs:
    typeof ord.curingCompletedAtMs === 'number' ? ord.curingCompletedAtMs : undefined,
  milestones: Array.isArray(ord.milestones) ? ord.milestones : [],
});

export const CURING_DURATION_MS = 3 * 24 * 60 * 60 * 1000; // 3 days

export const isReadyForCourier = (o: OrderRecord): boolean => {
...
export const getSavedOrders = (): OrderRecord[] => {
  try {
...
export const subscribeToOrders = (onChange: (orders: OrderRecord[]) => void): () => void => {
...
export const saveOrderRecord = (order: OrderRecord): void => {
  try {
    ...
    window.dispatchEvent(new CustomEvent('orders_updated', { detail: filtered }));
...
export const verifyOrderPayment = (orderNumber: string): OrderRecord | null => {
...
export const rejectPaymentByHost = (
  orderNumber: string,
  reason: string
): OrderRecord | null => {
  const orders = getSavedOrders();
  ...
  const order = { ...orders[idx] };
  const cleanReason = reason?.trim() || 'UTR / Payment reference not found in PhonePe statement';
  ...
  localStorage.setItem(STORAGE_KEY, JSON.stringify(orders));
  window.dispatchEvent(new CustomEvent('orders_updated', { detail: orders }));
  window.dispatchEvent(new CustomEvent('order_payment_rejected', { detail: order }));
  pushOrderToCloud(order);
  return order;
};

export const resubmitPaymentUtr = (
  orderNumber: string,
  newUtr: string
): OrderRecord | null => {
  const orders = getSavedOrders();
...
export const advanceOrderToCuring = (orderNumber: string): OrderRecord | null => {
  const orders = getSavedOrders();
  const idx = orders.findIndex((o) => o.orderNumber === orderNumber);
  if (idx === -1) return null;
...
export const markSoapCraftingDone = (orderNumber: string): OrderRecord | null => {
  const orders = getSavedOrders();
  const idx = orders.findIndex((o) => o.orderNumber === orderNumber);
  if (idx === -1) return null;
  ...
  order.milestones = [
    {
      ...
      description: 'Handcrafted botanical soaps cured & quality inspected. Packed and ready for courier pickup.',
      timestamp,
...
export const revertSoapCraftingToCuring = (orderNumber: string): OrderRecord | null => {
  const orders = getSavedOrders();
  const idx = orders.findIndex((o) => o.orderNumber === orderNumber);
  if (idx === -1) return null;
...
export const assignCourierTracking = (
  ...
  if (!url) {
    ...
    } else if (lowerCarrier.includes('blue dart') || lowerCarrier.includes('bluedart')) {
      url = `https://www.bluedart.com/tracking`;
    } else if (lowerCarrier.includes('india post') || lowerCarrier.includes('speed post')) {
...
export const findOrder = (query?: string | null): OrderRecord | undefined => {
  ...
  return orders.find((o) => {
    ...
    const cleanTrack = (o.trackingId || '').toLowerCase().replace(/[^a-z0-9]/g, '');
