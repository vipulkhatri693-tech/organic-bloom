...
import type { OrderRecord, ComplaintTicket, SoapProduct } from '../types';
...
export const isCloudSyncEnabled = (): boolean => isFirebaseConfigured && Boolean(db);

// ---------------- Orders ----------------

export const pushOrderToCloud = (order: OrderRecord): void => {
  ...
  try {
    ...
    setDoc(
      ...
      cleanPayload,
      { merge: false }
    ).catch((e) => console.error('[Organic Bloom] Failed to sync order to cloud:', e));
  } catch (e) {
...
export const subscribeToOrdersCloud = (
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        const orders = snapshot.docs
          .map((d) => d.data() as OrderRecord & { _updatedAt?: number })
          .sort((a, b) => (b._updatedAt || 0) - (a._updatedAt || 0));
        onChange(orders);
      },
      (err) => console.error('[Organic Bloom] Orders cloud subscription error:', err)
    );
  } catch (e) {
...
export const pushComplaintToCloud = (ticket: ComplaintTicket): void => {
  if (!isCloudSyncEnabled() || !db) return;
  try {
    setDoc(
      ...
      { merge: true }
    ).catch((e) => console.error('[Organic Bloom] Failed to sync complaint to cloud:', e));
  } catch (e) {
    console.error('[Organic Bloom] Failed to sync complaint to cloud:', e);
  }
};

export const subscribeToComplaintsCloud = (
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        const tickets = snapshot.docs
          .map((d) => d.data() as ComplaintTicket & { _updatedAt?: number })
          .sort((a, b) => (b._updatedAt || 0) - (a._updatedAt || 0));
        onChange(tickets);
      },
      (err) => console.error('[Organic Bloom] Complaints cloud subscription error:', err)
    );
  } catch (e) {
    console.error('[Organic Bloom] Failed to subscribe to complaints cloud:', e);
    return () => {};
  }
};
...
export const pushProductToCloud = (product: SoapProduct): void => {
  if (!isCloudSyncEnabled() || !db) return;
  try {
    setDoc(
      ...
      { merge: false }
    ).catch((e) => console.error('[Organic Bloom] Failed to sync product to cloud:', e));
  } catch (e) {
    console.error('[Organic Bloom] Failed to sync product to cloud:', e);
  }
};

export const deleteProductFromCloud = (productId: string): void => {
  if (!isCloudSyncEnabled() || !db) return;
  try {
...
export const replaceAllProductsInCloud = async (products: SoapProduct[]): Promise<void> => {
  if (!isCloudSyncEnabled() || !db) return;
  try {
    ...
    const deletions = snapshot.docs
      ...
      .map((d) => deleteDoc(d.ref).catch((e) => console.error('[Organic Bloom] Cleanup delete failed:', e)));
...
export const subscribeToProductsCloud = (
  onChange: (products: SoapProduct[]) => void
): Unsubscribe => {
  if (!isCloudSyncEnabled() || !db) {
    return () => {};
  }
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        const products = snapshot.docs.map((d) => {
          const data = d.data() as SoapProduct & { _updatedAt?: number };
          const { _updatedAt, ...rest } = data;
          return rest as SoapProduct;
        });
        onChange(products);
      },
      (err) => console.error('[Organic Bloom] Products cloud subscription error:', err)
    );
  } catch (e) {
...
export const pushAnnouncementToCloud = (announcement: {
  ...
  try {
    setDoc(
      ...
      { merge: false }
    ).catch((e) => console.error('[Organic Bloom] Failed to sync announcement to cloud:', e));
  } catch (e) {
    console.error('[Organic Bloom] Failed to sync announcement to cloud:', e);
  }
};

export const subscribeToAnnouncementCloud = (
  onChange: (announcement: { text: string; active: boolean; updatedAt: string } | null) => void
): Unsubscribe => {
  if (!isCloudSyncEnabled() || !db) {
    return () => {};
  }
  try {
    return onSnapshot(
      ...
      (snap) => {
        ...
        const { _updatedAt, ...rest } = snap.data() as any;
        onChange(rest);
      },
      (err) => console.error('[Organic Bloom] Announcement cloud subscription error:', err)
    );
  } catch (e) {
    console.error('[Organic Bloom] Failed to subscribe to announcement cloud:', e);
    return () => {};
  }
};
